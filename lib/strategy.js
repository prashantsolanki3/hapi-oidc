...
export const plugin = {
  ...
  register: async (server, options) => {
    ...
    if (!config.discoverUrl && MANUAL_SETTINGS.some((k) => !(k in config)))
      throw new Error('You must provide a discoverUrl or a valid manual settings');
    ...
    server.route({
      ...
      handler: async (request, h) => {
        ...
        try {
          const token = await client.authorizationCallback(config.callbackUrl, request.query, {
            state,
          });
